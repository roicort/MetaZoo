    ...
    def initialize(self):
        if self.encoding == "binary":
            return np.random.randint(
                0, 2, size=(self.population_size, self.genome_length)
            )
        elif self.encoding == "boolean":
            return np.random.choice(
                [True, False], size=(self.population_size, self.genome_length)
            )
        elif self.encoding == "real":
            if isinstance(self.bounds, (list, tuple)) and isinstance(
                self.bounds[0], (list, tuple)
            ):
                ...
                return np.random.uniform(
                    low, high, size=(self.population_size, self.genome_length)
                )
        else:
